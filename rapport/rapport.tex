...
Selon le modèle d'attaquant, on voit donc que pour se prémunir face à une \emph{Cache Timing Attack}, le fondeur devra utilliser des méthodes plus ou moins perfectionnées en fonction des moyens de l'attaquant.
...
Une difficulté primordiale de cette méthode est de pouvoir mesurer le temps d'accès aux données. 
...
\paragraph{présence de bruit:} rend difficile la récupération des traces du chiffrement et donc .. %TODO C
...
\paragraph{Implémentation d'un algorithme en temps constant}
- algo en temps constant = Implémentation d'AES résistante aux timing attacks~\cite{kasper2009faster}
