...
%TODO osvik: soit désactiver le cache, soit mode nofill
...
\section*{Conclusion}
L'utilisation des caches fournit des temps d'accès aux données variables, permettant à un attaquant d'obtenir des informations sur le matériel cryptographique. Un certain nombre de techniques visant à empêcher cette attaque ne font que la rendre plus difficile, et sont grandement dépendantes de l'architecture considérée. Par ailleurs, si l'implémentation d'algorithmes en temps constant permet en théorie d'empêcher l'attaque, ils sont en pratique difficiles à réaliser et ne présentent pas des temps d'exécution satisfaisants. %TODO finir La solution à la fois efficace et indépendante de architecture 
